---
aliases: ["Приём · ёмкость_с_диэлектриком", "ёмкость_с_диэлектриком"]
cssclasses: [ege-moc, ege-technique]
tags: [ege/moc/technique, ege/technique/ёмкость_с_диэлектриком]
technique: ёмкость_с_диэлектриком
---

# Приём · ёмкость_с_диэлектриком

> [!abstract] Описание
> Конденсатор с частично введённым диэлектриком

## 🗂 Задачи с этим приёмом

```dataview
TABLE WITHOUT ID file.link AS Задача, subject_ru AS Предмет, kes_code AS КЭС, difficulty AS Сл, status AS Статус
FROM #ege/technique/ёмкость_с_диэлектриком AND -#ege/moc
SORT difficulty DESC, date DESC
```

## 📊 Успешность по приёму

```dataview
TABLE WITHOUT ID key AS Предмет, length(rows) AS Всего, length(filter(rows, (r) => r.status = "pass")) AS ✓, length(filter(rows, (r) => r.status = "fail")) AS ✗
FROM #ege/technique/ёмкость_с_диэлектриком AND -#ege/moc
WHERE subject_ru
GROUP BY subject_ru
SORT key ASC
```
