---
aliases: ["Приём · переменный_поток", "переменный_поток"]
cssclasses: [ege-moc, ege-technique]
tags: [ege/moc/technique, ege/technique/переменный_поток]
technique: переменный_поток
---

# Приём · переменный_поток

> [!abstract] Описание
> Поток меняется по сложному закону
...
FROM #ege/technique/переменный_поток AND -#ege/moc
