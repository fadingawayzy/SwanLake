...
> Open-source ИИ-репетитор для ЕГЭ. Генерирует усложнённые версии задач из банка ФИПИ через LLM, проверяет ответы независимой моделью, складывает в Obsidian-vault со SRS-расписанием.

[![Python](https://img.shields.io/badge/python-3.10+-blue.svg)](https://www.python.org/)
[![Release](https://img.shields.io/github/v/tag/fadingawayzy/SwanLake?label=release)](https://github.com/fadingawayzy/SwanLake/releases)
[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](LICENSE)
[![Status](https://img.shields.io/badge/status-beta-orange.svg)]()

<img src="https://github.com/user-attachments/assets/8d2686d4-4d6e-4467-942a-bcc428abd665" alt="Граф знаний SwanLake — связи между предметами, КЭС и приёмами усложнения" />

## Кому это нужно

- **Школьникам 10–11 классов**, которые готовятся к ЕГЭ и хотят тренироваться на задачах сложнее банка ФИПИ.
- **Репетиторам**, которые хотят автоматически генерировать варианты с пошаговым решением для своих учеников.
- **Учителям-методистам**, которые хотят добавить свои приёмы усложнения через PR в `modification_framework.json`.

## Quickstart

```bash
git clone https://github.com/fadingawayzy/SwanLake.git
cd SwanLake
pip install -r requirements.txt
cp .env.example .env  # → впиши OPENROUTER_API_KEY
python seed_demo.py --reset && python moc.py  # посмотреть демо без API-вызовов
```

Открой `vault/` в Obsidian → `_moc/HOME.md`.

📖 Полная установка: [Установка](#установка) · 🏗 Архитектура: [ARCHITECTURE.md](ARCHITECTURE.md) · 🤖 Для ИИ-агентов: [CLAUDE.md](CLAUDE.md)

## Чем отличается от Умскула / Яндекс Репетитор AI / Winny

| | SwanLake | Закрытые SaaS |
|---|:---:|:---:|
| Open-source (MIT) | ✅ | ❌ |
| Локальный vault — данные у вас | ✅ | ❌ |
| Кросс-верификация двумя независимыми LLM | ✅ | ❌ |
| Свой ключ OpenRouter — все траты прозрачны | ✅ | ❌ |
| Своя методика через PR | ✅ | ❌ |
| Работает оффлайн после генерации | ✅ | ❌ |
| Цена за месяц подготовки | ~$5–30 (только API) | 1490–3990 ₽/мес |

> SwanLake — это инфраструктура для подготовки, а не онлайн-школа. Он не заменяет учителя, но снимает с него рутину генерации вариантов.

---

## ЕГЭ-202X — что внутри

Система подготовки к ЕГЭ по 4 предметам: **математика (профиль), физика, информатика, русский язык**.

Берёт реальные задания из открытого банка ФИПИ, переписывает их в более сложные варианты через LLM (OpenRouter), верифицирует ответы кросс-моделью, складывает в Obsidian-хранилище с Dataview-дашбордами и SRS (интервальное повторение по SM-2).
